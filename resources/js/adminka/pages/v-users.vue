    ...
    <transition name="slide">
        <div>
            ...
            <b-modal :id="infoModal.id" :title="infoModal.title" hide-footer @hide="resetInfoModal" size="lg">
                <v-user-edit :user="infoModal.content"></v-user-edit>
            </b-modal>
        </div>
    </transition>
</template>
...
<script>
    ...
    import vUserEdit from '../components/v-user-edit'
    import {mapActions} from 'vuex'

    export default {
        name: "v-users",
        components: {vUserEdit},
        data() {
            return {
                spinner: true,
                ...
                fields: [
                    ...
                    { key: 'email', sortable: true, label:'Почта' },
        ...
        methods: {
            ...
            info(item, index, button) {
                if(item.id)  {this.infoModal.title = `Редактирование пользователя: ${item.name}`;}
                else {this.infoModal.title = 'Создание пользователя'}
                        ...
                        if(value) {
                            axios.delete('/api/user/' + item.id )
                                .then((res) => {
                                    ...
                                    if(res.data.status)  {
                                        setTimeout(() => {this.GET_USERS()}, 1000);
                                    }
                                })
                                .catch((error) => {
                                    console.log(error);
