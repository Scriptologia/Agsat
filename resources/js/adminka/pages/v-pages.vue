        ...
        <div>
            ...
            ><template #table-busy>
                ...
                <template #cell(name)="row">
                    <a :href="'http://agsat/page/'+row.item.slug"
                       target="_blank">{{row.item.name_ru}}</a>
                </template>
                <template #cell(edit)="row">
                    <b-icon-pencil @click="info(row.item, row.index, $event.target)" variant="primary" type="button"></b-icon-pencil>
                </template>
                <template #cell(delete)="row">
                    <b-icon-trash variant="danger" @click="deletePage(row.item)" type="button"></b-icon-trash>
                </template>
            </b-table>
            ...
            <b-modal :id="infoModal.id" :title="infoModal.title" hide-footer @hide="resetInfoModal" size="lg" no-enforce-focus>
                <v-page-edit :page="infoModal.content"></v-page-edit>
            </b-modal>
        </div>
    </transition>
</template>
...
<script>
    import vPageEdit from '../components/v-page-edit'
    import {mapActions} from 'vuex'
    export default {
        name: "v-pages",
        components: {vPageEdit},
        data() {
            return {
                spinner: true,
                categories:[],
                ...
                fields: [
                    ...
                    { key: 'created_at', sortable: true, label:'Создана',
                        formatter: (value, key, item) => {
        ...
        methods: {
            ...
            info(item, index, button) {
                if(item.slug)  {this.infoModal.title = `Редактирование страницы: ${item.name_ru}`;}
                else {this.infoModal.title = 'Создание страницы'}
            ...
            deleteCategory(item) {
                this.$bvModal.msgBoxConfirm('Вы действительно хотите удалить?', {
                    ...
                    .then(value => {
                        if(value) {
                            axios.delete('/api/pages/' + item.slug )
                                .then((res) => {
                                    this.makeToast(true, res.data.message, res.data.status);
                                    if(res.data.status)  {
                                        setTimeout(() => {this.GET_PAGES()}, 1000);
                                    }
                                })
                                .catch((error) => {
                                    console.log(error);
        ...
        computed: {
            rows() {
                return this.$store.state.pages.length
            }
        }
    }
</script>
