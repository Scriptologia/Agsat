    ...
    <transition name="slide">
        <div>
            ...
            <b-modal :id="infoModal.id" :title="infoModal.title" hide-footer @hide="resetInfoModal" size="lg">
                <v-role-edit :role="infoModal.content" :permissions="$store.state.permissions" ></v-role-edit>
            </b-modal>
        </div>
    </transition>
</template>
...
<script>
    ...
    import vRoleEdit from '../components/v-role-edit'
    import {mapActions} from 'vuex'

    export default {
        name: "v-roles",
        components: {vRoleEdit},
        data() {
            return {
                spinner: true,
                ...
                fields: [
                    ...
                    { key: 'permissions', sortable: false, label:'Права',
                        formatter: (value, key, item) => {
                            return item.permissions.map(item => item.description).join(', ');
                        }
                    },
                    { key: 'edit',  label:'Ред.' },
                    { key: 'delete',  label:'Уд.' },
        ...
        methods: {
            ...
            info(item, index, button) {
                if(item.id)  {this.infoModal.title = `Редактирование роли: ${item.name}`;}
                else {this.infoModal.title = 'Создание роли'}
                        ...
                        if(value) {
                            axios.delete('/api/role/' + item.id )
                                .then((res) => {
                                    ...
                                    if(res.data.status)  {
                                        setTimeout(() => {this.GET_ROLES()}, 1000);
                                    }
                                })
                                .catch((error) => {
                                    console.log(error);
